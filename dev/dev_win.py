...
def init():
    ...
    run_cmd(
        [join(BKE_VENV_DIR, "Scripts", "python.exe"), "-m", "pip", "install", "-v", "-r", "requirements.txt"],
        cwd=BKE_DIR,
    ...
    run_cmd(["npm", "install"], cwd=FTE_DIR, shell=True)
...
def update_json_schemas():
    ...
    run_cmd(
        [
            join(BKE_VENV_DIR, "Scripts", "datamodel-codegen.exe"),
            "--input",
            DATA_SCHEMA_DIR,
            "--input-file-type",
            "jsonschema",
        ...
        cwd=BKE_DIR,
        env=dict(environ, VIRTUAL_ENV=BKE_VENV_DIR, PATH=join(BKE_VENV_DIR, "Scripts") + ":" + environ["PATH"]),
        shell=True,
    )
...
def deploy_android():
    ...
    run_cmd(["npm", "run", "debug-build"], cwd=FTE_DIR, shell=True)

    print("# Syncing Android code")
    run_cmd(["npx", "cap", "sync"], cwd=FTE_DIR, shell=True)

    print("# Generating APK")
    run_cmd(["npx", "cap", "build", "android"], cwd=FTE_DIR, shell=True)
